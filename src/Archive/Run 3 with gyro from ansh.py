...
def main():                                         # this is the main function. The code will flow top
    ...
    Drive.log_source_filter = 'LFLT'                        # while running only show 'TEST' logs
    ...
    gyro_spin_to_angle(-93)
    gyro_drive('s', target=82.5, speed=10)

    turn_left(200, 0.2)
    right_extension(1050, 4)
    move_forward_duration(-70,2)
    turn_right(400, 1.7)
    left_lift.run(-100, async_op=True)
    
    
    
    gyro_drive('d', 21, 30,                            # set drive parameters
    ...
    left_lift.run(10, 60)
    backwards_left(200, 1)
    backwards_right(200, 1)'''
    gyro_drive('d', -50, 100)
